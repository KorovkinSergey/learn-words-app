...
router.post("/dictionaries", auth, async (req: any, res: any) => {
	try {
		User.findById(req.user.id, async (err: any, user: any) => {
			...
			user.dictionary = user.dictionary.map((item: any) => item.title === "Новые слова" ? {
				...item,
				words: [
					...dictionary.words,
					...dictionaries.reduce((words: IWord[], name: string) => {
						const newWords = JSON.parse(readFileSync(path.join(__dirname, `../db/${name}.json`), "utf-8"))
							.filter(({ russian }: any) => !dictionary.words.find((el: any) => el.russian === russian));
