...
const Navigation = () => {
	...
	return (
		<Box sx={{ width: '100%' }}>
			<BottomNavigation sx={{ backgroundColor: 'secondary.light' }} showLabels onChange={handleChange}>
				{navigation.map(({ value, label, icon }) => (
					<BottomNavigationAction sx={{ color: 'white' }} key={value} value={value} label={label} icon={icon} />
				))}
			</BottomNavigation>
		</Box>
	)
